 ...
 */
    ...
    loadFromOBJ(fileText)
    {
        ...
        for (var i = 0; i < lines.length; i++) {
            if (lines[i].charAt(0) == '#') {
                ...
                this.fBuffer.push(parseInt(values[1]));
                this.fBuffer.push(parseInt(values[2]));
                this.fBuffer.push(parseInt(values[3]));
            }
        }
    ...
    */
    drawTriangles() {
        gl.bindBuffer(gl.ARRAY_BUFFER, this.VertexPositionBuffer);
        gl.vertexAttribPointer(shaderProgramMesh.vertexPositionAttribute, this.VertexPositionBuffer.itemSize, 
                         gl.FLOAT, false, 0, 0);

        // Bind normal buffer
        gl.bindBuffer(gl.ARRAY_BUFFER, this.VertexNormalBuffer);
        gl.vertexAttribPointer(shaderProgramMesh.vertexNormalAttribute, 
    ...
    */
    ...
    {  
        for(var i=0;i<this.numVertices;i++) {
            console.log("v ", this.vBuffer[i*3 + 0], " ", 
                              this.vBuffer[i*3 + 1], " ",
                              this.vBuffer[i*3 + 2], " ");
        ...
        for(var i=0;i<this.numFaces;i++) {
            console.log("f ", this.fBuffer[i*3 + 0], " ", 
                              this.fBuffer[i*3 + 1], " ",
                              this.fBuffer[i*3 + 2], " ");
    ...
    */   
    generateNormals() {
        ...
        for(var i=0;i<this.numFaces;i++) {
            ...
            var v1Vec = vec3.fromValues(this.vBuffer[3*v1+0],
                                        this.vBuffer[3*v1+1],
                                        this.vBuffer[3*v1+2]);
            var v2 = this.fBuffer[3*i+1]; 
            var v2Vec = vec3.fromValues(this.vBuffer[3*v2+0],
                                        this.vBuffer[3*v2+1],
                                        this.vBuffer[3*v2+2]);
            var v3 = this.fBuffer[3*i+2]; 
            var v3Vec = vec3.fromValues(this.vBuffer[3*v3+0],
                                        this.vBuffer[3*v3+1],
                                        this.vBuffer[3*v3+2]);
